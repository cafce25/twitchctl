use crate::api::{UserIdent, ApiClient};
    ...
    match command {
        TagsSubcommand::ListAll {
            shared: SharedTagsOptions { long, filter },
                ...
                Err(e) => {
                    eprintln!("An error occured while fetching the tags: {}", e);
                    exit(1);
                }
        }
        ...
        TagsSubcommand::List {
            ...
            let tags = client.get_stream_tags(get_broadcaster_id_or_die(&client, broadcaster, broadcaster_id).await);
            match tags.await {
                Ok(tags) => list(&tags, locale, filter, long),
                Err(e) => {
                    eprintln!("An error occured while fetching the tags: {}", e);
        ...
        TagsSubcommand::Set {
            ...
            let current_tags = async {
                if add {
                    client
                        .get_stream_tags(broadcaster_id.clone())
                        .await
                        .unwrap_or(vec![])
                } else {
                    vec![]
            ...
            let all_tags = match client
                .get_all_tags()
                .await {
                    Ok(all_tags) => all_tags,
                    Err(_) => {
                        eprintln!("twitch did not return valid tags to match against");
                        exit(1);
                    }
                };

            let mut new_tags: Vec<TwitchTag> = tags
                .iter()
                .filter_map(|tag| {
                    for tag_obj in all_tags.iter() {
                        match tag_obj.localization_names.get(locale) {
                            Some(loc_name) if loc_name == tag => return Some(tag_obj.clone()),
                            Some(_) => {},
                            _ => {
                                // maybe just do nothing?!?
                                eprintln!("Did not receive any localized data for locale {}", locale);
                                exit(1);
                            }
                        }
                    }
            ...
            match client
                .replace_stream_tags(
                    ...
                    .into_iter()
                    .filter_map(|t| {
                        if t.is_auto == AutoGenerated::True {
                            None
                        } else {
                            Some(t.id)
                        }
...
fn list(tags: &[TwitchTag], locale: &str, filter: Option<String>, long: bool) {
    ...
    let max_len = tags
        .iter()
        .map(|tag| {
            match tag.localization_names
                .get(locale)
                {
                    Some(localized_name) => localized_name.len(),
                        _ => {
                            eprintln!("The locale '{}' was not found in the tags returned by Twitch", locale);
                            exit(1);
                        }
                }
        })
